    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Gerenciamento de Planos.</p>
        </div>
        <Data />
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-3 rounded-2xl border border-slate-200/80 shadow-xs">
        ...
        <button
          className="w-full sm:w-auto px-4 py-2.5 bg-[#00828a] hover:bg-[#006e75] text-white text-sm font-semibold rounded-xl transition-all shadow-sm shadow-[#00828a]/20 flex items-center justify-center gap-2 cursor-pointer"
          onClick={() => setCadastroForm(true)}
        >
      ...
      <CadastroPlano
        isOpen={cadastroForm}
        onClose={() => setCadastroForm(false)}
        onSave={(dados) => console.log("Dados do Plano:", dados)}
      />
    </div>
  );
}